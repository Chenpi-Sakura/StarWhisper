...
All data complies with the GeoJSON format (provided in EPSG:4326, bounded to 
 and geometries valid as per ST_IsValid on GEOS 3.9.3).
...
ConstellationsPermalink
constellations.<data format> (POINT) with 89 objects. Additionally a constellations.csv file with the data (no coordinates) is also provided:
...
Traditional Chinese constellationsPermalink
constellations.cn.<data format> (POINT) with 312 objects. Additionally a constellations.cn.csv file with the data (no coordinates) is also provided:
...
display_ra, display_dec, display_scale: for single constellation display
Constellation boundariesPermalink
...
constellations.bounds.<data format> (MULTIPOLYGON) with 89 objects. See Constellation lines for field descriptions.

Traditional Chinese constellationsPermalink
constellations.borders.cn.<data format> (LINESTRING) with 84 objects.

ids: border id.
constellations.bounds.cn.<data format> (POLYGON) with 28 objects. See Constellation lines for field descriptions.

Constellation linesPermalink
...
Traditional Chinese constellationsPermalink
constellations.lines.cn.<data format> (MULTILINESTRING) with 255 objects1:
...
Traditional Chinese DSO namesPermalink
...
dsos.6.<data format>, dsos.14.<data format>, dsos.20.<data format>: the number indicates limit magnitude.
...
pop: MW populations OH|YH|BD (Old halo, young halo, bulge & disk), M31 populations M31|GP (gen. M31, great plane)
...
StarsPermalink
StarnamesPermalink
starnames.csv: Magnitude independent, all stars with a name/designation other than HIP/HD
...
Traditional Chinese star namesPermalink
...
pinyin: Pinyin transcription
StarsPermalink
stars.6.<data format>, stars.8.<data format>, stars.14.<data format>: the number indicates limit magnitude:
...
Some constellations have been removed from the original dataset since the MULTILINESTRING object consisted only on a single point, that does not conform with the GeoJSON standard (POINT as MULTILINESTRING are not valid). ↑

Blog
Stack Overflow
WikiCommons
GitHub
© 2025 Diego Hernangomez

Powered by Chulapa Jekyll Theme
GitDev Dark skin by dieghernan