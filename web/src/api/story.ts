...
export async function streamStory(
  ...
  const r = await fetch('/api/atlas-story/stream', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(req),
    signal,
...
/**
 * Photo-level 流式故事事件：沿用 P2-16 的 title / char / done，error 变体额外带
 * `code`。photo-level 不发 `reset`（AI 失败直接 error，无 preset 降级）。
 */
export type PhotoStreamEvent = StoryStreamEvent | PhotoErrorEvent

/**
 * `POST /api/story` —— photo-level 故事流（整张照片的多星座上下文交给 AI）。
 * 协议与 streamStory 相同（SSE 字符级），差别在请求 body 与 error 事件带 code。
 */
export async function streamPhotoStory(
  ...
  if (!r.ok || !r.body) {
    ...
    onEvent({
      type: 'error',
      code: body?.detail?.code ?? `HTTP_${r.status}`,
      message: body?.detail?.message ?? `HTTP ${r.status}`,
    })
    return
  }
  const reader = r.body.getReader()
