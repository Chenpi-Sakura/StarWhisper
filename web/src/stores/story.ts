...
export const useStoryStore = defineStore('story', () => {
  ...
  // 流式打字机状态（/api/story/stream SSE）
  const streaming = ref(false)
  const streamTitle = ref('')
  const streamParagraphs = ref<StreamParagraph[]>([])
  ...
  async function fetchStoryStream(
    ...
    // streamTitle/streamParagraphs。StoryPanel 的 displayParagraphs 会在
    // streamParagraphs 为空时回退到 current.paragraphs 一次性渲染。
    if (action === 'refetch' && isHit(abbr, style)) {
      return current.value as StoryResponse
    }

    // P0-5：中止上一个还在跑的流（连点视角 tab / 快速切星座时，
    // 两个 SSE 往同一数组 push 会段落交错）
    activeSeq += 1
    ...
    try {
      await streamStory(req, (ev) => {
        ...
        else if (ev.type === 'paragraph') {
          streamParagraphs.value.push({ index: ev.index, text: ev.text })
        } else if (ev.type === 'done') {
          finalMeta = ev.meta
          current.value = ev.meta
        } else if (ev.type === 'reset') {
          // P0-3：后端降级前先发 reset——清空半截 AI 内容，等 preset 全量重放
          streamTitle.value = ''
          streamParagraphs.value = []
  ...
  return {
    ...
    streaming, streamTitle, streamParagraphs, streamError,
